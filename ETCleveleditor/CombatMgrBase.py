...
##      ** queueAttack
##      *** Adjusted code to include wider range of naming conventions
##
##      07/26/2011 - Zachary Cummings - ZJC
##      ** queueAttack
##      *** Adjusted attack code to run dynamic attacks
##      *** specific to the name of the model/actor calling the attack
##
##
##############################################################################
...
#            targetObjBehaviors.removeAi('flock')
    ...
    def queueAttack(self, attackerObj, attackerSpell, targetObj):
        ...
        ## # ZJC - 07/29/2011: Code not comprehensive of all possible name formats, changes added below
        ## whoSpell = attackerObj.getName().split('_')[1]  # ZJC - 07/26/2011: Determines the 
                                                        ## # name of the character attacking, 
                                                        ## # assuming:    _NAME_
        
        
        whoSpell = attackerObj.getName().split('_')[0]        # ZJC - 07/29/2011: Assumes format is Name_mod:#
        ## print name
        for i in range(len(attackerObj.getName().split('_'))): # ZJC - 07/29/2011: Loop will check every possibility in the name string for any level of merges/imports
            whoSpell2 = attackerObj.getName().split('_')[i]   # ZJC - 07/29/2011: name2 holds the current piece of the name string 
            if ("mod:" in whoSpell2) and (whoSpell != whoSpell2):                          # ZJC - 07/29/2011: This means the name format is Prefix(es)_Name_mod:#
                whoSpell = sattackerObj.getName().split('_')[i-1] # ZJC - 07/29/2011:  Assigns correct model name, the one just before mod:#
        ...
        if attackerSpell.isCooldownReady():
            if self.combatGraph.checkCanAttack(attackerObj, targetObj):
                ...
                if dist > attackerSpell.getAttackRange():
                    ...
                    # ZJC - 07/26/2011: These three lines take the spell as referenced in the spell
                    # dictionary and append to them the name of the attacker to call the right animations
                    firstSpellAnim = whoSpell + attackerSpell.properties['chargeSequence']['firstAnimation']
                    secondSpellAnim = whoSpell + attackerSpell.properties['chargeSequence']['secondAnimation']
                    chargeIdleAnim = whoSpell + attackerSpell.properties['chargeSequence']['idleAnimation']
                    
                    
                    if isActor and attackerSpell.properties['chargeSequence']['firstAnimation'] != '':
                        charge.append(Func(proj.getOwner().getActorHandle().play, firstSpellAnim))      # ZJC - 07/26/2011: Replaced longer call with shortened name
                    ...
                    if isActor and attackerSpell.properties['chargeSequence']['secondAnimation'] != '':
                        charge.append(Func(proj.getOwner().getActorHandle().play, secondSpellAnim))     # ZJC - 07/26/2011: Replaced longer call with shortened name
                    ...
                    if isActor and attackerSpell.properties['chargeSequence']['idleAnimation'] != '':
                        charge.append(Func(proj.getOwner().getActorHandle().loop, chargeIdleAnim))      # ZJC - 07/26/2011: Replaced longer call with shortened name
                    
                    
                    
                    ## ZJC - 07/26/2011: This was the previous block, it has been commented out and left as
